...
class Shop(models.Model):
    ...
    state = models.BooleanField(verbose_name=_('Cтатус получения заказов'), default=True)
...
class Product(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("api:product", kwargs={'id': self.id})   # f'/api/v1/products-form/{self.id}/'


class ProductInfo(models.Model):
    """ Модель, содержащая информацию о товаре """
    ...
    def get_absolute_url(self):
        return reverse("api:product", kwargs={'id': self.product.id})   # f'/api/v1/products-form/{self.id}/'
...
class ProductParameter(models.Model):
    """ Модель, содержащая информацию о параметрах товара """
...
class Order(models.Model):
    ...
    status = models.CharField(verbose_name=_('Статус'), choices=STATUS_CHOICES, max_length=15)
